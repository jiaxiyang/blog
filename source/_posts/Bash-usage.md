...
## Miscellaneous
1. `ftp`: 搭建ftp服务器，脚本put:`scp -r $@ user@IP:path`上传文件，脚本get:`for i in $@; do scp -r user@IP:path/${i}; done`获取文件。get-ls:`ssh xiyang@xsjsda153 "cd /var/ftp/pub/xiyang/  && ls $@"`, get-list-all:`ssh user@IP "cd path && ls -alhrt"`, get-list:`ssh user@IP "cd path && ls -lhrt | tail -n 10"`获取ftp服务器列表。ftp服务器上可以放常用文件，各服务器方便取。也可用于服务器(包括windows)间数据交换。
